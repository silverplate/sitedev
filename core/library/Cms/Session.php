...
abstract class Core_Cms_Session
{
	...
	public static function GetId() {
		if (!isset($_COOKIE[self::Get()->GetCookieName()]) || !$_COOKIE[self::Get()->GetCookieName()]) {
			self::SetId(App_Db::Get()->GetUnique(self::PREFIX . 'session', self::PREFIX . 'session_id', 30));
		}
	...
	public function Login($_user_id, $_is_ip_match = false, $_life_span = null, $_timeout = null, $_valid_date = null) {
		...
		App_Db::Get()->Execute('UPDATE ' . self::PREFIX . 'session' . App_Db::Get()->GetQueryFields(array(
			...
			'is_logged_in' => 1,
			'user_id' => App_Db::escape($_user_id),
			'life_span' => $_life_span ? $_life_span : 0,
			'timeout' => $_timeout ? $_timeout : 0,
			'valid_date' => $_valid_date ? App_Db::escape(date('Y-m-d H:i:s', $_valid_date)) : 'NULL'
	...
	public static function Clean($_user_id = null) {
		App_Db::Get()->Execute('
			DELETE FROM
				' . self::PREFIX . 'session
			WHERE
				' . ($_user_id ? 'user_id = ' . App_Db::escape($_user_id) . ' OR ' : '') . '
		...
		$sessions = App_Db::Get()->GetList('SELECT ' . self::PREFIX . 'session_id FROM ' . self::PREFIX . 'session');
		if ($sessions) {
			App_Db::Get()->Execute('DELETE FROM ' . self::PREFIX . 'session_param WHERE ' . self::PREFIX . 'session_id NOT IN (' . App_Db::escape($sessions) . ')');
		} else {
			App_Db::Get()->Execute('TRUNCATE ' . self::PREFIX . 'session_param');
		}
	}
