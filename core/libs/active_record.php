...
//                 array_push($conditions, "$attribute = ''");
                ...
                array_push($conditions,
                           $attribute . ' IN (' . Db::escape($value) . ')');

            } else {
                array_push($conditions,
                           $attribute . ' = ' . Db::escape($value));
            }
        }
    ...
    public function addAttribute($_name,
                                 ...
                                 $_isPrimary = false,
                                 $_isUnique = false) {
        $this->Attributes[$_name] = new ActiveRecordAttribute(
            $_name, $_type, $_length, $_isPrimary, $_isUnique
        );
    }

    public function addForeignKey(ActiveRecord $_obj, $_isPrimary = false)
    {
    ...
    public function GetPrimaryKeyStatment($_value = null) {
        ...
        if ($key) {
            if (is_array($key)) {
                for ($i = 0; $i < count($key); $i++) {
                    if ($_value && is_array($_value)) {
                        if (isset($_value[$key[$i]->GetName()])) {
                            $value = Db::escape($_value[$key[$i]->GetName()]);
                        } elseif (isset($_value[$i])) {
                            $value = Db::escape($_value[$i]);
                        } else {
                ...
                $value = ($_value) ? Db::escape($_value) : $key->GetValue();
...
//                         $item->SetValue(Db::Get()->GetNextNumber($this->GetTable(), $item->GetName()));
    ...
    public function update(array $_fields = null) {
        $fields = empty($_fields)
                ? Db::get()->getQueryFields($this->getAttributeValues(true), 'update', true)
                : Db::get()->getQueryFields($_fields, 'update');
...
//             if (isset($_data[$item->GetName()])) {
    ...
    public static function IsUnique($_class, $_table, $_pk, $_attribute, $_value, $_exclude = null) {
        return !(self::GetList(
            ...
            array('count' => 1),
            is_null($_exclude) ? null : array($_pk . ' != ' . Db::escape($_exclude))
        ));
    }
